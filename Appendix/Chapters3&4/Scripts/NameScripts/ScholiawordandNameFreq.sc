...
@main
def nameHistogram(urn: String, srcFile: String) {
    ...
    finalPrint(urn,mainHistogram,intHistogram,ilHistogram,extHistogram,imHistogram)
}
...
def finalPrint(urn: String,mainHistogram: Vector[(String, Double)],intHistogram: Vector[(String, Double)],ilHistogram: Vector[(String, Double)],extHistogram: Vector[(String, Double)],imHistogram: Vector[(String, Double)]) = {

    val mainResult = mainHistogram.filter(_._1 == urn)
    val intResult = intHistogram.filter(_._1 == urn)
    val ilResult = ilHistogram.filter(_._1 == urn)
    val extResult = extHistogram.filter(_._1 == urn)
    val imResult = imHistogram.filter(_._1 == urn)
