//This script aligns a text of interest with an index of morphological parses in order to globally replace the words of the text with their lemmata. The script requires two inputs. The first is the file containing the text to be normalized, formatted in two columns with the first column containing CTS URNs for the text and the second column containing the text itself. This first input should be the same input as the text that was used for the frist script in the m-normalization process. The second input is  the three-columned index of morphological parses that was created by the second step of the m-normalization process, wherein the first column contains the word analyzed by the Morpheus parser, the second column contains the parts of the speech the word could be recognized as, and the third contains the possible lemmata of the word as determined by Morpheus. The result of this script is another version of the text, p-normalized, which is identical in format to the text that was used as the input. So it will contain two columns where the first column contains the CTS URN for a scholion and the second column contains the text of the scholion but with every word replaced with its lemma or lemmata. 
